...
#	test "DateTime valid",   do: assert DateTime.from_iso8601( "2018-06-04T16:01:09Z" ) == { :ok, #DateTime<2018-06-04 16:01:09Z>, 0 }
	test "DateTime invalid", do: assert DateTime.from_iso8601( "2018-02-30T16:01:09Z" ) == { :error, :invalid_date }

	test "NaiveDateTime valid",   do: assert NaiveDateTime.new( 2010, 1, 13, 23, 10, 7, 5000 ) == { :ok, ~N[2010-01-13 23:10:07.005000] }
	test "NaiveDateTime invalid", do: assert NaiveDateTime.new( 2010, 2, 30, 23, 10, 7, 5000 ) == { :error, :invalid_date }
	test "NaiveDateTime to_erl",  do: assert NaiveDateTime.to_erl( ~N[2010-01-13 23:10:07.005000] ) == { { 2010, 1, 13 }, { 23, 10, 7 } }
end

defmodule PrimitiveTimexTest do
	use PowerAssert

	test "Timex is_valid DateTime",          do: assert Timex.is_valid?( ~N[2010-01-13 23:10:07.005000] ) == true
	test "Timex is_valid Date",              do: assert Timex.is_valid?( ~D[2018-06-04] ) == true
	test "Timex is_valid Tuple DateTime",    do: assert Timex.is_valid?( { { 2010, 1, 13 }, { 23, 10, 7 } } ) == true

	test "Timex is_valid String Date invalid",           do: assert Timex.is_valid?( "2018-06-04" ) == false
	test "Timex is_valid String Date Slash invalid",     do: assert Timex.is_valid?( "2018/06/04" ) == false
	test "Timex is_valid String DateTime invalid",       do: assert Timex.is_valid?( "2018-06-04 23:10:07" ) == false
	test "Timex is_valid String Slash DateTime invalid", do: assert Timex.is_valid?( "2018/06/04 23:10:07" ) == false
	test "Timex is_valid String DateTime TZ invalid",    do: assert Timex.is_valid?( "2018-06-04T23:10:07Z" ) == false
